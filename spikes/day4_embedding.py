...
def main() -> None:
    ...
    def load(repo: str, number: int, variant: str):
        ...
        print(f"  #{number} [{variant}]: {len(hunks)} hunks / "
    ...
    for (repo, role), (s, ha, hb, a, b) in results.items():
        i, j = s["argmax"]
        print(f"{repo:9s}{role:16s}{s['max']:8.4f}{s['mean_top3']:8.4f}{s['n_pairs']:9d}"
              f"   #{a} {ha[i][0]} ↔ #{b} {hb[j][0]}")
    ...
    HEADLINE = {"fastapi": ("similar_a", "similar_b_src"),
                "p5js":    ("similar_a", "similar_b_src")}
    ...
    for repo in ("fastapi", "p5js"):
        ...
        for (r, role) in results:
            ...
            print(f"  {repo:9s}{role:16s} MAX {s['max'] - ctrl['max']:+.4f}"
    ...
    for label, fa, p5 in (
        ...
        print(f"  {label:34s} fastapi {per_repo['fastapi'][fa]:+.4f}   "
